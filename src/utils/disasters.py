...
class AsteroidStrike:
    ...
    def draw_laser(self):
        if 0 < self.timeout < 3 * ASTEROID_TIME_TICS / 4:
...
class Disasters:
    def __init__(self, parent):
        self.tic = None
        self.destroyed_locs = None
        self.asteroid_strike = None
        self.dust_storm = None
        self.warnings = None
        self.parent = parent
    ...
    def draw_warnings(self):
        for building in self.warnings:
            arcade.draw_lrwh_rectangle_textured(building.center_x - 116 / 2, building.center_y - 82 / 2,
                                                116, 82, self.texture_warn,
                                                alpha=255 * math.fabs(math.sin(0.05 * self.tic)))
